...
(async () => {
  ...
  let { dumpJson, comlibs, hasPermissionFn, headTags, executeEnv } =
  ...
  if (headTags && decodeURIComponent(headTags)) {
    // 插入 headTags 到 html 中
    const headTagsStr = decodeURIComponent(headTags);
    const headTagsDom = document.createElement("div");
    headTagsDom.innerHTML = headTagsStr;
    const headTagsDomChildren = headTagsDom.children;

    for (let i = 0; i < headTagsDomChildren.length; i++) {
      // 插入 到 html 中
      if (headTagsDomChildren[i].tagName === "SCRIPT") {
        if (headTagsDomChildren[i].src) {
          await new Promise((resolve) => {
            const script = document.createElement("script");
            script.src = headTagsDomChildren[i].src;
            script.onload = resolve;
            document.head.appendChild(script);
          });
        } else {
          // 如果是 script 标签，需要特殊处理
          const script = document.createElement("script");
          script.innerHTML = headTagsDomChildren[i].innerHTML;
          document.head.appendChild(script);
        }
      } else {
        document.head.appendChild(headTagsDomChildren[i]);
      }
    }
  }
